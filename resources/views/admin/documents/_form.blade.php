...
<div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
    ...
    <div class="lg:col-span-2">
        ...
        <select name="fund_id" id="fund_id"
            ...
            @foreach($funds as $fund)
                <option value="{{ $fund->id }}" {{ old('fund_id', $document->fund_id ?? '') == $fund->id ? 'selected' : '' }}>
                    {{ $fund->name }}
                </option>
            @endforeach
        </select>
    ...
    {{-- Title --}}
    <div>
        <label for="title" class="mb-1 block text-sm font-medium text-gray-700">Title <span
                class="text-red-500">*</span></label>
    ...
    {{-- Category --}}
    <div>
        <label for="category" class="mb-1 block text-sm font-medium text-gray-700">Category <span
                class="text-red-500">*</span></label>
        <select name="category" id="category"
            class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-red-500 focus:outline-none focus:ring-1 focus:ring-red-500"
            required>
            <option value="">Select a category</option>
            @foreach(['monthly', 'factsheet', 'prospectus', 'other'] as $category)
    ...
    {{-- Publish Date --}}
    <div>
        <label for="publish_date" class="mb-1 block text-sm font-medium text-gray-700">Publish Date <span
                class="text-red-500">*</span></label>
