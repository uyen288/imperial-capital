...
<div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <select name="fund_id" id="fund_id"
            ...
            @foreach($funds as $fund)
                <option value="{{ $fund->id }}"
                    {{ old('fund_id', $portfolio->fund_id ?? '') == $fund->id ? 'selected' : '' }}>
                    {{ $fund->name }}
                </option>
            @endforeach
        </select>
    ...
    {{-- Ticker --}}
    <div>
        <label for="ticker" class="mb-1 block text-sm font-medium text-gray-700">Ticker <span class="text-red-500">*</span></label>
        <input type="text" name="ticker" id="ticker"
               value="{{ old('ticker', $portfolio->ticker ?? '') }}"
               class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-red-500 focus:outline-none focus:ring-1 focus:ring-red-500"
               required>
    </div>
    ...
    <div>
        ...
        <input type="text" name="sector" id="sector"
               value="{{ old('sector', $portfolio->sector ?? '') }}"
               class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-red-500 focus:outline-none focus:ring-1 focus:ring-red-500">
    </div>
